...
class DB_Cache:
    ...
    async def add(self, key, value):
        if self.cache_dict.get(key) != value:
            if self.cache_dict.get(key) is None:
                log.debug(f"Adding value for {key} in {self.table}")
            else:
                log.debug(f"Changing {key} in {self.table}")
    ...
    async def get(self, key):
        if self.cache_dict.get(key) is None:
            cache_item = await db.select(self.value_name, self.table, self.key_name, key)

            if cache_item is None:
                log.debug(f"Count not find {self.value_name} in table {self.table} for {self.key_name} = {key}")
                return None
            else:
                log.debug(f"Found {self.value_name} in table {self.table} for {self.key_name} = {key}. Loading into cache")
    ...
    async def flush_cache_item_to_db(self, key):
        ...
        db_item = await db.select(self.value_name, self.table, self.key_name, key)
